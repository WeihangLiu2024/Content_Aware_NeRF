...
class GridEncoder(nn.Module):
    ...
    def forward(self, inputs, bound=1, max_level=None, out_embeddings=None):
        ...
        if out_embeddings is None:
            outputs = grid_encode(inputs, self.embeddings, self.offsets, self.per_level_scale,
                                  self.base_resolution, inputs.requires_grad, self.gridtype_id, self.align_corners,
                                  self.interp_id, max_level)
        else:
            outputs = grid_encode(inputs, out_embeddings, self.offsets, self.per_level_scale,
                                  self.base_resolution, inputs.requires_grad, self.gridtype_id, self.align_corners,
                                  self.interp_id, max_level)
    ...
    def size_down(self, optimizer, scheduler):
        ...
        offset = 0
        # self.log2_hashmap_size = self.log2_hashmap_size - 1 + (self.log2_hashmap_size-12) / 8
        self.log2_hashmap_size = self.log2_hashmap_size - 2 * (1 - (self.log2_hashmap_size - 12) / 8)
        self.max_params = round(2 ** self.log2_hashmap_size)
        # self.log2_hashmap_size = self.log2_hashmap_size - 1
        # self.max_params = 2 ** self.log2_hashmap_size
        for i in range(self.num_levels):
