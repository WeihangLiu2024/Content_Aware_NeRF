...
class NeRFRenderer(nn.Module):
    def __init__(self, opt):
        ...
        self.cascade = max(1 + math.ceil(math.log2(self.bound)), 1)
    ...
    # ==== two quantized pipeline ====
    def calibration_run_cuda(self, rays_o, rays_d, bg_color=None, perturb=False, cam_near_far=None,
                             shading='full', **kwargs):
        # rays_o, rays_d: [N, 3]
        # return: image: [N, 3], depth: [N]
        ...
        while step < self.opt.max_steps:
            ...
            dir_norm = torch.norm(dirs, dim=-1, keepdim=True).squeeze(dim=1)
            dirs[dir_norm != 0, :] = dirs[dir_norm != 0, :] / dir_norm[dir_norm != 0].unsqueeze(1)

            with torch.cuda.amp.autocast(enabled=self.opt.fp16):
                outputs = self.calibration(xyzs, dirs)
                sigmas = outputs['sigma']
                rgbs = outputs['color']
    ...
    def quantize_run_cuda(self, rays_o, rays_d, bg_color=None, perturb=False, cam_near_far=None, shading='full',
                          **kwargs):
        # rays_o, rays_d: [N, 3]
        # return: image: [N, 3], depth: [N]
    ...
    # ==== two quantized render ====
    # render = raymarching + model_forward
    def calibration_render(self, rays_o, rays_d, **kwargs):
        # TODO: raymarching is not quantized
        # TODO: "run" has no quantized version, i.e., CPU version is not implemented
        # rays_o, rays_d: [N, 3]
        # return: pred_rgb: [N, 3]

        return self.calibration_run_cuda(rays_o, rays_d, **kwargs)

    def quantize_render(self, rays_o, rays_d, **kwargs):
        # TODO: raymarching is not quantized
        # TODO: "run" has no quantized version
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: pred_rgb: [B, N, 3]

        if self.cuda_ray:
            return self.quantize_run_cuda(rays_o, rays_d, **kwargs)
        elif self.training:
            return self.run(rays_o, rays_d, **kwargs)
        else:  # staged inference
            N = rays_o.shape[0]
